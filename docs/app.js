...
function buildIntel(){
  ...
  $('dupes').innerHTML=groups.length?groups.map(group=>{
    ...
    const specialtyTextA=normalizeSearch(`${specialtyA} ${specialtyEnA}`),specialtyTextB=normalizeSearch(`${specialtyB} ${specialtyEnB}`),specialtyTokensA=new Set(specialtyTextA.split(' ').filter(token=>token.length>2)),specialtyOverlap=[...specialtyTokensA].filter(token=>specialtyTextB.includes(token)),specialtyMatch=Boolean(specialtyTextA&&specialtyTextB&&(specialtyTextA===specialtyTextB||specialtyOverlap.length>=2)),specialtyState=specialtyMatch?'هم‌راستا':specialtyTextA&&specialtyTextB?'متفاوت؛ نیازمند بررسی':'داده ناکافی';
