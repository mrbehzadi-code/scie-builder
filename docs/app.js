...
function setupBulkImport(){
  ...
  $('bulkImportFile').addEventListener('change',async event=>{const file=event.target.files?.[0];if(!file)return;try{const text=await file.text(),raw=file.name.toLowerCase().endsWith('.json')?JSON.parse(text):parseCsv(text),rows=Array.isArray(raw)?raw:Array.isArray(raw.records)?raw.records:Array.isArray(raw.people)?raw.people:[];prepared=rows.slice(0,500).map(normalize);draw();$('bulkImportStatus').textContent='پیش‌نمایش آماده است؛ پس از بررسی، ثبت نهایی را بزنید.'}catch(error){prepared=[];draw();$('bulkImportStatus').textContent=`خواندن فایل ناموفق بود: ${error.message}`}});
