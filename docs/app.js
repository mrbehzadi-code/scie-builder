...
let people=[],filtered=[],page=1,q='',src='all',cat='all',operationFilter='',snap={},INT={},ER={},PROFILE={},EXT={},ENT={},KG={},LOC={},SOCIAL={},REL={},BUILD={},MERGES=[],MERGE_REQUESTS=[],SOURCE_MONITORS={monitors:[],changes:[]},RECORD_VERSIONS=[];
...
function monitorStatusFa(status){return ({healthy:'سالم',changed:'تغییر یافته',unavailable:'خارج از دسترس',queued:'در صف پایش'}[status]||status||'نامشخص')}
function renderSourceMonitors(){
  if(!$('sourceMonitorList'))return;
  if(authUser()?.role!=='admin'){
    $('sourceMonitorStats').innerHTML='';
    $('sourceMonitorList').innerHTML='<div class="admin-inbox-lock"><i>◌</i><strong>پایش منابع در اختیار مدیر سامانه است</strong><span>برای مشاهده تغییرات منابع، خطاهای دسترسی و زمان بررسی بعدی با حساب مدیر وارد شوید.</span><button type="button" class="button primary" data-source-login>ورود مدیر</button></div>';
    return;
  }
  const monitors=SOURCE_MONITORS.monitors||[],changes=SOURCE_MONITORS.changes||[],counts={healthy:0,changed:0,unavailable:0,queued:0};
  monitors.forEach(item=>counts[item.last_status]=(counts[item.last_status]||0)+1);
  $('sourceMonitorStats').innerHTML=`<span><b>${monitors.length.toLocaleString('fa-IR')}</b> منبع ثبت‌شده</span><span class="healthy"><b>${counts.healthy.toLocaleString('fa-IR')}</b> سالم</span><span class="changed"><b>${changes.length.toLocaleString('fa-IR')}</b> تغییر نیازمند بررسی</span><span class="unavailable"><b>${counts.unavailable.toLocaleString('fa-IR')}</b> خارج از دسترس</span>`;
  const pendingByRecord=new Map(changes.map(item=>[Number(item.record_index),item]));
  const rows=[...monitors].sort((a,b)=>{const rank={changed:0,unavailable:1,queued:2,healthy:3};return (rank[a.last_status]??4)-(rank[b.last_status]??4)}).slice(0,40);
  $('sourceMonitorList').innerHTML=rows.length?rows.map(item=>{const change=pendingByRecord.get(Number(item.record_index)),checked=item.last_checked_at?new Date(item.last_checked_at).toLocaleString('fa-IR'):'هنوز بررسی نشده',next=item.next_check_at?new Date(item.next_check_at).toLocaleString('fa-IR'):'در اولین نوبت';return `<article class="source-monitor-item status-${esc(item.last_status)}"><span class="source-monitor-signal">${item.last_status==='healthy'?'✓':item.last_status==='changed'?'↻':item.last_status==='unavailable'?'!':'◌'}</span><div class="source-monitor-copy"><span>${esc(item.source_type||'web')} · رکورد ${Number(item.record_index).toLocaleString('fa-IR')}</span><strong>${esc(item.person_name)}</strong><a href="${esc(item.source_url)}" target="_blank" rel="noopener">${esc(item.source_url)}</a><small>آخرین بررسی: ${esc(checked)} · بررسی بعدی: ${esc(next)}${item.error_message?` · ${esc(item.error_message)}`:''}</small></div><b>${esc(monitorStatusFa(item.last_status))}</b><div class="source-monitor-actions"><button type="button" data-source-record="${Number(item.record_index)}">پرونده فرد</button><a href="${esc(item.source_url)}" target="_blank" rel="noopener">منبع اصلی ↗</a>${change?`<button type="button" class="accept" data-source-change="${esc(change.id)}" data-source-action="accept">تأیید تغییر</button><button type="button" class="dismiss" data-source-change="${esc(change.id)}" data-source-action="dismiss">نادیده‌گرفتن</button>`:''}</div></article>`}).join(''):'<div class="operation-empty">هنوز منبعی برای پایش ثبت نشده است. «ثبت منابع فعلی» را بزنید؛ پس از آن بررسی‌ها خودکار و زمان‌بندی‌شده ادامه پیدا می‌کند.</div>';
}
async function loadSourceMonitors(){
  if(authUser()?.role!=='admin'){SOURCE_MONITORS={monitors:[],changes:[]};renderSourceMonitors();return}
  try{SOURCE_MONITORS=await adminFetch('/admin/source-monitors');renderSourceMonitors();buildOperations()}catch(error){$('sourceMonitorStatus').textContent=error.message}
}
...
async function runSourceMonitoring(){
  if(!adminToken()){openAdminLogin(runSourceMonitoring);return}
  $('runSourceMonitors').disabled=true;$('sourceMonitorStatus').textContent='در حال بررسی منابع سررسیدشده…';
  try{const result=await adminFetch('/admin/source-monitors/run',{method:'POST',body:JSON.stringify({limit:40})}),message=`${Number(result.checked||0).toLocaleString('fa-IR')} منبع بررسی شد؛ ${Number(result.changed||0).toLocaleString('fa-IR')} تغییر و ${Number(result.unavailable||0).toLocaleString('fa-IR')} خطای دسترسی.`;$('sourceMonitorStatus').textContent=message;if(result.changed||result.unavailable)addAppNotification({id:`source-${Date.now()}`,type:'source',title:'نتیجه پایش منابع آماده است',message,created_at:new Date().toISOString(),target:{tab:'operations',selector:'.source-monitor-center'}});await loadSourceMonitors()}catch(error){$('sourceMonitorStatus').textContent=error.message}finally{$('runSourceMonitors').disabled=false}
}
async function reviewSourceChange(id,action){
  $('sourceMonitorStatus').textContent='در حال ثبت تصمیم مدیر…';
  try{await adminFetch('/admin/source-monitors/review',{method:'POST',body:JSON.stringify({id,action})});$('sourceMonitorStatus').textContent=action==='accept'?'تغییر منبع تأیید و برای پیگیری ثبت شد.':'تغییر از صف بررسی خارج شد.';await loadSourceMonitors()}catch(error){$('sourceMonitorStatus').textContent=error.message}
}
const auditFieldFa=field=>({name:'نام انگلیسی',name_fa:'نام فارسی',type:'نوع ظرفیت',source:'منبع',verification:'وضعیت تأیید',organization_fa:'سازمان فارسی',affiliation:'سازمان / وابستگی',location:'مکان',specialty:'حوزه تخصصی',specialty_en:'تخصص انگلیسی',specialty_evidence:'شاهد تخصص',specialty_source_url:'منبع تخصص',detail:'جزئیات',url:'نشانی منبع'}[field]||field);
function renderAuditTrail(){
  if(!$('auditTimeline'))return;
  if(authUser()?.role!=='admin'){$('auditStats').innerHTML='';$('auditTimeline').innerHTML='<div class="admin-inbox-lock"><i>↶</i><strong>دفتر تغییرات ویژهٔ مدیر است</strong><span>با ورود مدیر، تاریخچهٔ کامل و امکان بازگردانی امن نمایش داده می‌شود.</span><button type="button" class="button primary" data-audit-login>ورود مدیر</button></div>';return}
  const records=new Set(RECORD_VERSIONS.map(item=>item.record_index)),rollbacks=RECORD_VERSIONS.filter(item=>item.action==='rollback').length;
  $('auditStats').innerHTML=`<span><b>${RECORD_VERSIONS.length.toLocaleString('fa-IR')}</b> نسخه ثبت‌شده</span><span><b>${records.size.toLocaleString('fa-IR')}</b> رکورد دارای تغییر</span><span class="changed"><b>${rollbacks.toLocaleString('fa-IR')}</b> بازگردانی انجام‌شده</span><span class="healthy"><b>۱۰۰٪</b> قابل حسابرسی</span>`;
  $('auditTimeline').innerHTML=RECORD_VERSIONS.length?RECORD_VERSIONS.slice(0,80).map(item=>{const person=people[Number(item.record_index)],name=person?bilingualName(person).persian:`رکورد ${Number(item.record_index).toLocaleString('fa-IR')}`;return `<article class="audit-item ${item.action}"><i>${item.action==='rollback'?'↶':'✎'}</i><div><span>${esc(auditFieldFa(item.field))} · ${new Date(item.changed_at).toLocaleString('fa-IR')}</span><strong>${esc(name)}</strong><p><del>${esc(item.previous_value||'بدون مقدار قبلی')}</del><b>←</b><ins>${esc(item.new_value||'مقدار خالی')}</ins></p><small>عامل تغییر: ${esc(item.changed_by_name||item.changed_by||'مدیر سامانه')}</small></div><div class="audit-actions"><button type="button" data-audit-record="${Number(item.record_index)}">مشاهده پرونده</button>${item.action==='edit'?`<button type="button" class="rollback" data-audit-rollback="${esc(item.id)}">بازگردانی این تغییر</button>`:''}</div></article>`}).join(''):'<div class="operation-empty">از این پس هر ویرایش جدید با نسخهٔ قبل و بعد در این دفتر ثبت می‌شود.</div>';
}
async function loadAuditTrail(){if(authUser()?.role!=='admin'){RECORD_VERSIONS=[];renderAuditTrail();return}try{const result=await adminFetch('/admin/record-versions');RECORD_VERSIONS=result.versions||[];renderAuditTrail()}catch(error){$('auditStatus').textContent=error.message}}
async function rollbackVersion(id){if(!confirm('مقدار این فیلد به نسخهٔ قبل بازگردانده شود؟ این اقدام نیز در تاریخچه ثبت خواهد شد.'))return;$('auditStatus').textContent='در حال بازگردانی نسخه…';try{const result=await adminFetch('/admin/record-versions/rollback',{method:'POST',body:JSON.stringify({id})}),person=people[Number(result.record_index)];if(person)person[result.field]=result.value;applyIntelligence();render();buildIntel();$('auditStatus').textContent='نسخهٔ قبلی با موفقیت بازگردانده و فوراً در داشبورد اعمال شد.';await loadAuditTrail()}catch(error){$('auditStatus').textContent=error.message}}
...
function buildOperations(){
  ...
  const pendingLeads=readLocalArray('scie_leads').filter(item=>!['completed','failed'].includes(item.status)).length,notifications=appNotifications().filter(item=>!item.read).length,sourceAlerts=(SOURCE_MONITORS.changes||[]).length+(SOURCE_MONITORS.monitors||[]).filter(item=>item.last_status==='unavailable').length;
  const queue=[
    ...
    {tone:'cyan',icon:'◌',count:sourceAlerts,title:'پایش تغییرات منابع',text:'تغییر محتوای منابع و خطاهای دسترسی که به بررسی مدیر نیاز دارند.',action:'monitor',label:'مشاهده پایش'}
  ];
  ...
  buildAdminInbox();renderSourceMonitors();
}

function adminInboxItems(){
  const items=[];
  MERGE_REQUESTS.filter(item=>item.status==='pending').forEach(item=>{const a=people[Number(item.primary_index)]||{},b=people[Number(item.duplicate_index)]||{};items.push({id:`merge-${item.id}`,type:'merge',priority:Number(item.confidence)>=85?'high':'medium',title:'تصمیم دربارهٔ ادغام دو رکورد',summary:`${bilingualName(a).persian} و ${bilingualName(b).persian}`,meta:`پیشنهاددهنده: ${item.proposed_by_name||'کاربر اطلس'} · اطمینان ${Number(item.confidence||0).toLocaleString('fa-IR')}٪`,created_at:item.created_at,merge:item})});
  people.filter(person=>person._merged_into===undefined&&['insufficient','possible'].includes(person._locality?.status)).slice(0,20).forEach(person=>items.push({id:`identity-${person._record_index}`,type:'identity',priority:person._locality?.status==='insufficient'?'high':'medium',title:'بررسی انتساب هویتی و ارتباط با اردکان',summary:bilingualName(person).persian,meta:`امتیاز ارتباط ${Number(person._locality?.score||0).toLocaleString('fa-IR')} از ۱۰۰ · ${person.source||'منبع نامشخص'}`,record:person}));
  people.filter(person=>person._merged_into===undefined&&(!person.specialty||!(person.affiliation||person.organization))).slice(0,15).forEach(person=>items.push({id:`profile-${person._record_index}`,type:'profile',priority:'medium',title:'تکمیل اطلاعات حرفه‌ای',summary:bilingualName(person).persian,meta:`${!person.specialty?'تخصص نامشخص':''}${!person.specialty&&!(person.affiliation||person.organization)?' · ':''}${!(person.affiliation||person.organization)?'سازمان نامشخص':''}`,record:person}));
  readLocalArray('scie_leads').filter(item=>!['completed','failed'].includes(item.status)).slice(0,15).forEach(item=>items.push({id:`lead-${item.id}`,type:'lead',priority:'normal',title:'پیگیری معرفی به اطلس',summary:item.value||'معرفی بدون عنوان',meta:`وضعیت: ${statusFa(item.status||'processing')} · ثبت‌شده در این دستگاه`,lead:item,created_at:item.created_at}));
  const rank={high:0,medium:1,normal:2};return items.sort((a,b)=>rank[a.priority]-rank[b.priority]||String(b.created_at||'').localeCompare(String(a.created_at||'')));
}

function buildAdminInbox(){
  if(!$('adminInboxList'))return;const user=authUser(),isAdmin=user?.role==='admin';
  if(!isAdmin){$('adminInboxStats').innerHTML='';$('adminInboxList').innerHTML=`<div class="admin-inbox-lock"><i>◇</i><strong>صندوق بررسی ویژهٔ مدیر است</strong><span>برای مشاهده و تصمیم‌گیری دربارهٔ پیشنهادها با حساب مدیر وارد شوید.</span><button type="button" class="button primary" data-inbox-login>ورود مدیر</button></div>`;return}
  const all=adminInboxItems(),filter=$('adminInboxFilter').value||'all',items=filter==='all'?all:all.filter(item=>item.type===filter),high=all.filter(item=>item.priority==='high').length;
  $('adminInboxStats').innerHTML=`<span><b>${all.length.toLocaleString('fa-IR')}</b> مورد باز</span><span class="urgent"><b>${high.toLocaleString('fa-IR')}</b> اولویت بالا</span><span><b>${MERGE_REQUESTS.filter(item=>item.status==='pending').length.toLocaleString('fa-IR')}</b> تصمیم ادغام</span>`;
  $('adminInboxList').innerHTML=items.length?items.map(item=>`<article class="admin-inbox-item priority-${item.priority}" data-inbox-id="${esc(item.id)}"><i>${esc({merge:'⇄',identity:'⌖',profile:'▣',lead:'＋'}[item.type])}</i><div class="admin-inbox-copy"><span>${esc({merge:'ادغام رکورد',identity:'راستی‌آزمایی هویت',profile:'تکمیل پروفایل',lead:'معرفی به اطلس'}[item.type])}</span><strong>${esc(item.title)}</strong><b>${esc(item.summary)}</b><small>${esc(item.meta)}</small></div><em>${item.priority==='high'?'فوری':item.priority==='medium'?'مهم':'عادی'}</em><div class="admin-inbox-actions">${item.type==='merge'?`<button type="button" class="approve" data-inbox-merge="approve" data-merge-id="${esc(item.merge.id)}">تأیید و اعمال</button><button type="button" class="reject" data-inbox-merge="reject" data-merge-id="${esc(item.merge.id)}">رد پیشنهاد</button><button type="button" data-inbox-open-merge>مشاهده مقایسه</button>`:`<button type="button" data-inbox-record="${item.record?._record_index??''}" data-inbox-type="${item.type}">${item.type==='lead'?'مشاهده پیگیری':'بازکردن پرونده'}</button>`}</div></article>`).join(''):'<div class="operation-empty">موردی با این فیلتر در انتظار بررسی نیست.</div>';
}
...
function openOperationalList(kind){
  ...
  if(kind==='monitor'){openTab('operations');setTimeout(()=>document.querySelector('.source-monitor-center')?.scrollIntoView({behavior:'smooth',block:'start'}),180);return}
...
function articleItems(person){return (Array.isArray(person?.articles)?person.articles:[]).map(item=>typeof item==='string'?{title:item}:{title:item?.title||item?.display_name||'',year:item?.year||item?.publication_year,url:item?.url||item?.doi||'',cited_by_count:item?.cited_by_count}).filter(item=>item.title)}
function articlesSection(person){const articles=articleItems(person),sourceUrl=person?.articles_source_url||person?.url||person?.source_url||'';return `<section class="detail-section profile-articles"><h3>آثار و مقالات</h3><div class="profile-article-list">${articles.length?articles.map((item,index)=>`<article><span>${(index+1).toLocaleString('fa-IR')}</span><div><b lang="en" dir="ltr">${esc(item.title)}</b><small>${item.year?`سال ${Number(item.year).toLocaleString('fa-IR')}`:'سال انتشار ثبت نشده'}${item.cited_by_count!==undefined?` · ${Number(item.cited_by_count).toLocaleString('fa-IR')} استناد`:''}</small></div>${item.url?`<a href="${esc(item.url)}" target="_blank" rel="noopener" aria-label="مشاهده مقاله">↗</a>`:''}</article>`).join(''):`<div class="article-empty"><b>عنوان اثر عمومی ثبت نشده است</b><span>${person?.articles_status==='no_public_works_found'?'پروفایل علمی بررسی شد اما اثر عمومی قابل نمایش پیدا نشد.':'این رکورد پروفایل OpenAlex قابل بازیابی ندارد یا هنوز غنی‌سازی نشده است.'}</span></div>`}</div>${sourceUrl?`<a class="all-articles-link" href="${esc(sourceUrl)}" target="_blank" rel="noopener">مشاهده همه آثار در منبع اصلی ↗</a>`:''}</section>`}

function explainableTrust(person){
  const quality=person._quality||{},profile=person._profile||{},locality=person._locality||{},ids=profile.identifiers||{},evidenceCount=(person.evidence||[]).length+(profile.evidence||[]).length;
  const qualityScore=Math.max(0,Math.min(100,Number(quality.score||0))),completeness=Math.max(0,Math.min(100,Number(profile.profile_completeness||0))),localityScore=Math.max(0,Math.min(100,Number(locality.score||0)));
  const stable=[ids.orcid,ids.openalex_id,ids.github_username,person.openalex_id,person.url||person.source_url].filter(Boolean),sourceSignals=new Set([person.source,profile.provider,person.url||person.source_url,...(person.evidence||[])].filter(Boolean));
  const humanValid=locality.status==='confirmed_by_human'&&!locality.stale_human_review,identityRaw=humanValid?100:ids.orcid?95:stable.length>=2?82:stable.length===1?62:person._entity_id?55:25;
  const parts=[
    {key:'evidence',label:'کیفیت شواهد',weight:30,raw:qualityScore,detail:`امتیاز لایهٔ کیفیت ${qualityScore.toLocaleString('fa-IR')} از ۱۰۰؛ ${evidenceCount.toLocaleString('fa-IR')} شاهد در پرونده ثبت شده است.`},
    {key:'identity',label:'قطعیت هویت',weight:25,raw:identityRaw,detail:humanValid?'بازبینی انسانی معتبر و منطبق با همین هویت ثبت شده است.':`${stable.length.toLocaleString('fa-IR')} شناسه یا نشانی پایدار برای تطبیق هویت در دسترس است.`},
    {key:'locality',label:'ارتباط با اردکان',weight:20,raw:localityScore,detail:`امتیاز ارتباط محلی ${localityScore.toLocaleString('fa-IR')} از ۱۰۰ است؛ وضعیت: ${statusFa(locality.status)}.`},
    {key:'complete',label:'کامل‌بودن پروفایل',weight:15,raw:completeness,detail:`پوشش فیلدهای مورد انتظار ${completeness.toLocaleString('fa-IR')}٪ است؛ ${(profile.missing_fields||[]).length.toLocaleString('fa-IR')} فیلد ناقص گزارش شده است.`},
    {key:'sources',label:'تنوع و استقلال منابع',weight:10,raw:Math.min(100,sourceSignals.size*25),detail:`${sourceSignals.size.toLocaleString('fa-IR')} نشانهٔ منبع مستقل یا قابل پیگیری در پرونده وجود دارد.`}
  ];
  parts.forEach(part=>part.points=Math.round(part.raw*part.weight/100));
  const warnings=[];let penalty=0;
  if(locality.stale_human_review){penalty+=20;warnings.push('تأیید انسانی قدیمی با هویت فعلی ناسازگار است.')}
  if(['needs_review','low'].includes(person.verification)||person.confidence==='low'){penalty+=8;warnings.push('رکورد در منبع اصلی نیازمند بازبینی یا کم‌اطمینان علامت‌گذاری شده است.')}
  if(!person.specialty){penalty+=4;warnings.push('حوزهٔ تخصصی هنوز ثبت نشده است.')}
  if(!(person.affiliation||person.organization||profile.organization)){penalty+=4;warnings.push('وابستگی سازمانی هنوز ثبت نشده است.')}
  const score=Math.max(0,Math.min(100,parts.reduce((sum,part)=>sum+part.points,0)-penalty));
...
function openDetail(person){
  ...
  const localized=bilingualName(person),organizationSource=person.affiliation||person.organization||profile.organization||'',organization=bilingualOrganization(organizationSource,person.organization_fa||person.affiliation_fa),avatar=avatarFor(localized.persian),trust=explainableTrust(person),qualityScore=trust.score,localityScore=Math.max(0,Math.min(100,Number(locality.score||0))),completeness=Math.max(0,Math.min(100,Number(profile.profile_completeness||0)));
  ...
  $('dTrust').innerHTML=`<div class="trust-ring" style="--score:${qualityScore}"><strong>${faDigits(qualityScore)}</strong><span>از ۱۰۰</span></div><b>امتیاز اعتماد</b><small>${esc(trust.level)}</small>`;
...
function buildIntel(){
  ...
  $('dupes').innerHTML=groups.length?groups.map(group=>{
    ...
    const specialtyA=String(first.specialty||first._profile?.specialty||''),specialtyB=String(second.specialty||second._profile?.specialty||''),specialtyEnA=String(first.specialty_en||first._profile?.specialty_en||''),specialtyEnB=String(second.specialty_en||second._profile?.specialty_en||''),specialtyEvidenceA=String(first.specialty_evidence||first.detail||'شاهد تخصصی ثبت نشده است'),specialtyEvidenceB=String(second.specialty_evidence||second.detail||'شاهد تخصصی ثبت نشده است'),specialtyStatusA=String(first.specialty_status||'ثبت مستقیم یا طبقه‌بندی اولیه'),specialtyStatusB=String(second.specialty_status||'ثبت مستقیم یا طبقه‌بندی اولیه');
    const specialtyTextA=normalizeSearch(`${specialtyA} ${specialtyEnA}`),specialtyTextB=normalizeSearch(`${specialtyB} ${specialtyEnB}`),specialtyTokensA=new Set(specialtyTextA.split(' ').filter(token=>token.length>2)),specialtyOverlap=[...specialtyTokensA].filter(token=>specialtyTextB.includes(token)),specialtyMatch=Boolean(specialtyTextA&&specialtyTextB&&(specialtyTextA===specialtyTextB||specialtyOverlap.length>=2)),specialtyState=specialtyMatch?'هم‌راستا':specialtyTextA&&specialtyTextB?'متفاوت؛ نیازمند بررسی':'داده ناکافی',articlesA=articleItems(first),articlesB=articleItems(second);
    ...
    const signals=[['نام فارسی',nameMatch?'مطابق':'نیازمند تطبیق',nameMatch],['حوزه تخصصی',specialtyState,specialtyMatch],['سازمان',orgMatch?'مطابق':orgA&&orgB?'متفاوت':'داده ناکافی',orgMatch],['منابع',independentSources?'دو منبع مستقل':sourceA||sourceB||'ثبت نشده',Boolean(sourceA||sourceB)],['تعارض‌ها',conflicts.length?conflicts.join('، '):'تعارض آشکار یافت نشد',!conflicts.length]];
    const rawUrlA=String(first.url||first.source_url||first._profile?.source_url||''),rawUrlB=String(second.url||second.source_url||second._profile?.source_url||''),urlA=/^https?:\/\//i.test(rawUrlA)?rawUrlA:'',urlB=/^https?:\/\//i.test(rawUrlB)?rawUrlB:'',specialtyUrlA=/^https?:\/\//i.test(String(first.specialty_source_url||''))?String(first.specialty_source_url):urlA,specialtyUrlB=/^https?:\/\//i.test(String(second.specialty_source_url||''))?String(second.specialty_source_url):urlB;
    ...
    const comparison=[['نام',a.persian,b.persian,nameMatch],['نام لاتین',a.english||'ثبت نشده',b.english||'ثبت نشده',Boolean(a.english&&b.english&&normalizeSearch(a.english)===normalizeSearch(b.english))],['تخصص فارسی',specialtyA||'ثبت نشده',specialtyB||'ثبت نشده',specialtyMatch],['تخصص انگلیسی',specialtyEnA||'ثبت نشده',specialtyEnB||'ثبت نشده',specialtyMatch],['سازمان',orgA||'ثبت نشده',orgB||'ثبت نشده',orgMatch],['مکان',locationA,locationB,normalizeSearch(locationA)===normalizeSearch(locationB)],['منشأ کشف',sourceA||'ثبت نشده',sourceB||'ثبت نشده',sourceA===sourceB],['شناسه',idA,idB,String(idA)===String(idB)]];
...
function bindEvents(){
  ...
  $('registerSourceMonitors').addEventListener('click',registerSourceMonitors);$('runSourceMonitors').addEventListener('click',runSourceMonitoring);
  $('sourceMonitorList').addEventListener('click',event=>{if(event.target.closest('[data-source-login]')){openAdminLogin();return}const decision=event.target.closest('[data-source-change]');if(decision){reviewSourceChange(decision.dataset.sourceChange,decision.dataset.sourceAction);return}const record=event.target.closest('[data-source-record]');if(record){const person=people[Number(record.dataset.sourceRecord)];if(person)openDetail(person)}});
  $('refreshAudit').addEventListener('click',loadAuditTrail);$('auditTimeline').addEventListener('click',event=>{if(event.target.closest('[data-audit-login]')){openAdminLogin();return}const rollback=event.target.closest('[data-audit-rollback]');if(rollback){rollbackVersion(rollback.dataset.auditRollback);return}const record=event.target.closest('[data-audit-record]');if(record&&people[Number(record.dataset.auditRecord)])openDetail(people[Number(record.dataset.auditRecord)])});
...
async function boot(){
  ...
  try{
    ...
    leads();loadMergeRequests();loadSourceMonitors();loadAuditTrail();
