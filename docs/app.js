...
let people=[],filtered=[],page=1,q='',src='all',cat='all',snap={},INT={},ER={},PROFILE={},EXT={},ENT={},KG={},LOC={},BUILD={};
...
async function loadLayers(){
  const targets=[['INT','intelligence.json'],['ER','entity_resolution.json'],['PROFILE','profile_enrichment.json'],['EXT','external_enrichment.json'],['ENT','entities.json'],['KG','knowledge_graph.json'],['LOC','locality_assessment.json']];
  const results=await Promise.allSettled(targets.map(([,file])=>loadJSON(file,{preferRaw:true})));
  const state={};
  results.forEach((result,index)=>{const [name,file]=targets[index];if(result.status==='fulfilled'){({INT,ER,PROFILE,EXT,ENT,KG,LOC}={INT,ER,PROFILE,EXT,ENT,KG,LOC,[name]:result.value});state[file]=true}else{console.warn(`${file} unavailable:`,result.reason);state[file]=false}});
  return state;
}
...
function setupVerification(){
  ...
  $('vrSubmit').addEventListener('click',()=>{const index=Number($('vrPerson').value),person=people[index];if(!person)return;send({id:Date.now(),kind:'locality_review',record_index:index,person_name:person.name,verdict:$('vrVerdict').value,reason:$('vrReason').value,note:$('vrNote').value.trim()},'vrStatus',$('vrSubmit'))});
  $('relSubmit').addEventListener('click',()=>{const a=people[Number($('relA').value)],b=people[Number($('relB').value)];if(!a||!b||a===b){$('relStatus').textContent='دو فرد متفاوت را انتخاب کنید.';return}send({id:Date.now(),kind:'relationship',person_a:a.name,person_b:b.name,relation:$('relType').value,note:$('relNote').value.trim()},'relStatus',$('relSubmit'))});
}
...
async function leads(){
  ...
  const draw=()=>{$('leadHistory').innerHTML=read().map(item=>{const run=findRun(item),failed=item.status==='failed',done=run?.status==='completed',added=Number(run?.new_records||0),state=done?'تکمیل‌شده':failed?'ارسال ناموفق':item.status==='submitting'?'در حال ارسال':'در حال پردازش',finished=done&&run.completed_at?new Date(run.completed_at).toLocaleString('fa-IR'):'';return `<div class="lead-item"><div class="lead-item-head"><strong>${esc(item.type_label||item.type)} · ${esc(item.value)}</strong><span class="lead-state ${done?'done':'pending'}">${state}</span></div><small>${esc(item.location||'بدون مکان')} · ${new Date(item.at||item.created_at).toLocaleString('fa-IR')}</small>${done?`<small class="lead-result">${added?`${esc(added)} نامزد جدید به فهرست افزوده شد`:'پردازش کامل شد؛ نتیجهٔ تازه‌ای پیدا نشد'}${finished?` · پایان: ${esc(finished)}`:''}</small>`:''}${!done&&!failed?'<small>وضعیت هر ۱۰ ثانیه خودکار بررسی می‌شود.</small>':''}${failed?`<small class="lead-result">${esc(item.error||'ارسال ناموفق بود؛ دوباره تلاش کنید.')}</small>`:''}${item.note?`<small>${esc(item.note)}</small>`:''}</div>`}).join('')||'<div class="empty-state"><strong>هنوز سرنخی ثبت نشده است</strong><small>سرنخ نخست را برای جستجو و اعتبارسنجی ارسال کنید.</small></div>'};
  ...
  $('save').addEventListener('click',async()=>{
    ...
    try{
      ...
      $('leadStatus').textContent='سرنخ ارسال شد؛ جستجو و اعتبارسنجی خودکار آغاز شد.';$('lv').value='';$('ln').value='';draw();startPolling();
