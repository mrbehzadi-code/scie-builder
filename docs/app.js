...
function renderProvenance(person){
  ...
  $('provenanceGraph').innerHTML=stages.map((stage,index)=>`<article class="provenance-node ${stage.state}"><i>${stage.icon}</i><div><small>مرحله ${index+1}</small><strong>${esc(stage.title)}</strong><span>${esc(stage.text)}</span><em>${esc(stage.meta)}</em></div></article>`).join('');
