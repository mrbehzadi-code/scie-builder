...
async function logActivity(env,user,kind,summary,payload){if(!user)return;await env.DB.prepare('INSERT INTO activities(id,user_id,kind,summary,payload_json,created_at) VALUES(?,?,?,?,?,?)').bind(crypto.randomUUID(),user.id,kind,summary,JSON.stringify(payload||{}),new Date().toISOString()).run()}
